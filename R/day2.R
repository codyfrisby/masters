...
for(i in 1:n) {
  ...
  # merge thursday scores with simulated friday scores:
  df.sim <- merge(df.sim, thur)
  df.sim$friday <- df.sim$friday + df.sim$shots
  ...
  rm(probs3); rm(probs4); rm(round4); rm(score2) 
  rm(score3); rm(score4)
}
######## END of Simulation ###############
# now run some tests.  
...
# save these results
write.csv(df, paste("data/modelResults/round2", gsub(":", ".", Sys.time()), 
