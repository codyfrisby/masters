...
fri <- read.csv("data/fri.csv")
rm(temp) # done with this now
dfnotcut <- df.sim[df.sim$name %in% fri$name, ]
dfnotcut <- merge(dfnotcut, fri)
...
for(i in 1:n) {
  ...
  dfnotcut$sunday <- dfnotcut$sunday + dfnotcut$friday
  ...
  rm(finish1); rm(round4); rm(score3); rm(score4)
}
######## END of Simulation ###############
# now run some tests.  
rm(list = ls())
...
write.csv(df, paste("data/modelResults/round3", gsub(":", ".", Sys.time()), 
...
### can we compare our updated odds with vegas'?
vegas <- read.table("data/vegasroundtwo", header = FALSE)
vegas$name <- paste0(vegas$V2, ", ", vegas$V1)
vegas <- vegas[, 3:4]
names(vegas)[1] <- "vegasodds"
x <- vegas$vegasodds
d <- as.numeric(sub(".*:", "", x))
n <- as.numeric(sub(":.", "", x))
vegas$vegasranks <- rank(n / d, ties.method = "min")
test <- merge(df, vegas, by.x = "name", by.y = "name")
