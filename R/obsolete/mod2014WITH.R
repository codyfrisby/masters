...
temp <- read.csv("data/Player probabilities - model 6 2014.csv")
names <- names(temp)
names(temp) <- c(names[-(39:40)], "pwith", "pwithout")
rm(names)
masters <- read.csv("~/Documents/masters/data/ranks2014.csv")
masters <- masters[order(masters$name), ]
modtest <- numeric()
...
print(system.time(for(i in 1:n) {
  ...
  modtest[i] <- cor(masters$rank, finish1$rank, method = "spearman")   
...
vegas <- read.csv("~/Documents/masters/data/vegasodds2014.csv")
