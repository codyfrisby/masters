...
temp <- read.csv("~/Documents/masters/data/player probabilties model 3.csv")
names <- names(temp)
names(temp) <- c(names[-(38:39)], "pwith", "pwithout")
rm(names)
...
holes_noeagle <- c(3, 4, 5, 6, 7, 9, 10, 11, 14, 16, 17, 18)
holes_eagle <- c(2, 8, 13, 15) # eagle possible
# remove the columns that cause a problem with tidyr::spread
temp$rtp_score <- NULL; temp$score <- NULL; temp$pwith <- NULL
df <- tidyr::spread(temp, Level, pwithout) # now we have a "wide" table
...
write.csv(finish, paste("~/Documents/masters/data/sims/finish", 
                        i, ".csv", sep = ""))
rm(finish); rm(df.sim); rm(probs1); rm(probs2)
#rm(list = ls())
###########################  END of SIMULATIONS ######################
hist(modtest, col = "green", main = "10000 Spearman's Rho", 
     xlab = "Model 3 Without Player Effect")
print(quantile(modtest, c(0.025, 0.975)))
print(summary(modtest))
