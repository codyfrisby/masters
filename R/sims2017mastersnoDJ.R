...
# save these results
write.csv(df, paste("data/modelResults/noDJ", gsub(":", ".", Sys.time()), 
...
# we could compare us to vegas here:
vegas <- read.csv("data/vegas2017odds.csv")
vegas <- data.table(vegas)
names(vegas) <- c("vegasprobs", "name")
vegas$vegasranks <- rank(-vegas$vegasprobs, ties.method = "min")
test <- merge(df, vegas, by.x = "name", by.y = "name")
test <- test[order(test$modranks), ]
message(paste("Agreement between vegas and ss:", 
              round(cor(test$prob, test$vegasprobs, method = "spearman"), 
                    4)))
