...
# this is the field for 2017, updated 1-29-17.  
players <- read.csv("~/Documents/masters/data/field2017.csv")
# subset the data with those that are actually playing this year.
col.names <- c("Player.Name", "Hole..", "Par", "P.triple.", "P.double.", 
               "P.bogey.", "P.par.", "P.birdie.", "P.eagle.")
# only want the players who are playing in 2017
...
df.sim <- df[!duplicated(df[c("Hole..", "Player.Name")]) | 
             ...
             col.names]
#df.sim <- df[df$Tournament.Year == 2015 & df$Round.. == 1, col.names]
rm(df)
