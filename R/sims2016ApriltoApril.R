...
temp <- read.csv("data/Player probabilities - 2016 SG april to april.csv")
names <- names(temp)
# renaming stuff
...
for(i in 1:n) {
  ...
  ### end playoff ###
  write.table(finish1, "data/sims/finish2016.csv", append = TRUE, sep = ",",
...
finish <- read.csv("data/sims/finish2016.csv", header = F)
...
masters <- read.csv("data/ranks2016.csv")
vegas <- read.csv("data/vegasodds2016.csv")
vegas <- data.table(vegas)
vegas$vegasranks <- rank(vegas$vegasodds, ties.method = "min")
...
message(paste("Agreement between vegas and us:", 
              round(cor(test$modranks, test$vegasranks, method = "spearman"), 
                    4)))
message(paste("Agreement between us and actual:", 
              round(cor(test$modranks, test$rank, method = "spearman"), 
                    4)))
message(paste("Agreement between vegas and actual:", 
              round(cor(test$vegasranks, test$rank, method = "spearman"), 
                    4)))
