...
temp <- read.csv("data/Player probabilities Model 6b - 2016.csv")
names <- names(temp)
...
df[, c("-1", "0", "1", "2")] <- NULL
df.sim <- df; rm(df)
df.sim <- df.sim[, c("name", "round", "par", "P.double", "P.bogey", 
...
probs1 <- df.sim[df.sim$round == 1, 
                 c("P.double", "P.bogey", "P.par", "P.birdie", "P.eagle")]
